libname pro "y:\liulei\collaboration\zheng cheng\prodata";
PROC IMPORT OUT= WORK.pro 
            DATAFILE= "y:\liulei\paper\paper20\pro.csv" 
            DBMS=CSV REPLACE;
     GETNAMES=YES;
     DATAROW=2; 
RUN;
...
/* likelihood for repeated measures */
	...
	loglik1=-.5*(pro-mu1)**2/vare-.5*log(vare);
		...
		do k=1 to 10;
  	    ...
  	    mu3= alpha1 * trt +gamma1 * a  + gamma2 *b;
		...
		if lastid=1 then do;
			...
			if death=1 then do; 				/* for death event */
				base_haz_d=h1 * event1 + h2 * event2 + h3 * event3 + h4 * event4 + h5 * event5 + 
					h6 * event6 + h7 * event7 + h8* event8 +h9 * event9 + h10 * event10;
				mu4= alpha1 * trt + eta * pro  + gamma1 *a + gamma2 *b;	
				loglik=loglik + mu4 + log(base_haz_d);
			end;
		end;
...
set est1 est0e;
run;
* Model II with interaction of pro and random intercept, but no interaction of pro and random slope;
proc nlmixed data=six3 qpoints=5 corr cov;
	parms/data=est0f;
...
/* likelihood for repeated measures */
	...
	loglik1=-.5*(pro-mu1)**2/vare-.5*log(vare);
		...
		do k=1 to 10;
			/* cumulative baseline hazard for time dependent measure */
			sum2=sum2 + baseh{k} * dur{k} * exp(eta * pro +   delta1 * pro * a);
		end;
		...
		if lastid=1 then do;
			...
			if death=1 then do; 				/* for death event */
				base_haz_d=h1 * event1 + h2 * event2 + h3 * event3 + h4 * event4 + h5 * event5 + 
					h6 * event6 + h7 * event7 + h8* event8 +h9 * event9 + h10 * event10;
				mu4= alpha1 * trt + eta * pro  + delta1 * pro * a + gamma1 *a + gamma2 *b;	
				loglik=loglik + mu4 + log(base_haz_d);
			end;
		end;
...
set est2 est0g;
run;
* Model II with covariance of random intercept and slope;
* Model II with interaction of pro and random intercept, and interaction of pro and random slope;
...
/* likelihood for repeated measures */
	...
	loglik1=-.5*(pro-mu1)**2/vare-.5*log(vare);
		...
		do k=1 to 10;
			/* cumulative baseline hazard for time dependent measure */
			sum2=sum2 + baseh{k} * dur{k} * exp(eta * pro +   delta1 * pro * a +  delta2 * pro * b);
		end;
		...
		if lastid=1 then do;
			...
			if death=1 then do; 				/* for death event */
				base_haz_d=h1 * event1 + h2 * event2 + h3 * event3 + h4 * event4 + h5 * event5 + 
					h6 * event6 + h7 * event7 + h8* event8 +h9 * event9 + h10 * event10;
				mu4= alpha1 * trt + eta * pro  + delta1 * pro * a +  delta2 * pro * b + gamma1 *a + gamma2 *b;	
				loglik=loglik + mu4 + log(base_haz_d);
			end;
		end;
...
run;
* Model I without covariance of random intercept and slope;
************************** Used in paper ****************************;
data est4c;
set est4;
if parameter="delta1" then delete;
if parameter="delta2" then delete;
run;
...
model id ~ general(loglik);
random a b ~ normal([0, 0], [vara, 0, varb]) subject=id;
ods output ParameterEstimates=est5 FitStatistics=fit5; 
estimate 'Mediation' beta1 * eta;
estimate 'eta+gamma' eta + gamma1;
predict pro-a out=b;
predict a out=a;
