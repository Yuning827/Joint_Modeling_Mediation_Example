library(sas7bdat)
library(survival)
res1=res2=res3=sd1=sd2=sd3=matrix(data=0,nrow=21,ncol=2)
sd1t=sd2t=sd3t=rep(0,21)
for (i in 1:20){
t=i
...
sd1[i+1,]=(apply(tmpb1[,c(2:3)],2,quantile,probs=0.75,na.rm=TRUE)-apply(tmpb1[,c(2:3)],2,quantile,probs=0.25,na.rm=TRUE))/(qnorm(0.75)-qnorm(0.25))
sd2[i+1,]=(apply(tmpb2[,c(2:3)],2,quantile,probs=0.75,na.rm=TRUE)-apply(tmpb2[,c(2:3)],2,quantile,probs=0.25,na.rm=TRUE))/(qnorm(0.75)-qnorm(0.25))
...
sd1t[i+1]=quantile(tmpb1[,2]+tmpb1[,3],probs=0.75,na.rm=TRUE)-quantile(tmpb1[,2]+tmpb1[,3],probs=0.25,na.rm=TRUE)
sd2t[i+1]=quantile(tmpb2[,2]+tmpb2[,3],probs=0.75,na.rm=TRUE)-quantile(tmpb2[,2]+tmpb2[,3],probs=0.25,na.rm=TRUE)
...
fit<-coxph(Surv(T2DEATH/30,DEATH)~RANDGRP,data=data)
   bh=basehaz(fit,centered=FALSE)
    S0=exp(-bh[,1])^(exp(fit$coef))
    S1=exp(-bh[,1])^(exp(fit$coef*2))
