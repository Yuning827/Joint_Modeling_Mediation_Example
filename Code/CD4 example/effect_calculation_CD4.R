...
v1wo_asy<-as.matrix(read.csv("cov1.csv",na.strings=".",header=FALSE)[,-1])
v2wo_asy<-as.matrix(read.csv("cov2.csv",na.strings=".",header=FALSE)[,-1])
par1<-read.csv("res1.csv",header=FALSE)
...
med1wo<-function(qq,par,K=20,M=50,t,nr){
    ...
    beta=par[16:23]
    eta=par[26]
    Sigma_a=matrix(data=c(par[27],par[29],par[29],par[28]),nrow=2,ncol=2)
    vare=par[30]
    gamma=par[24:25]
    NDE<-NIE<-rep(NA,nr)
    for (xi in 1:nr){
...
med2wo<-function(qq,par,K=20,M=50,t,nr){
   ...
   alpha=par[11:15]
    beta=par[16:23]
    eta=par[26]
    Sigma_a=matrix(data=c(par[29],par[31],par[31],par[30]),nrow=2,ncol=2)
    vare=par[32]
    gamma=par[24:25]
    delta=par[27:28]
    NDE<-NIE<-rep(NA,nr)
