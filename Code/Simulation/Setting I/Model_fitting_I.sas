libname cheng "y:\liulei\collaboration\zheng cheng\paper2\data4";
%let datadir=y:\liulei\collaboration\zheng cheng\paper2\data4;
...
parms 	h01=0.025 h02=0.05 h03=0.075 h04=0.1 
		alpha1=1 beta0=0 beta1=1 beta2=.2 gamma=1 eta=0.5  vara=1 vare=1 ;
...
if aa=2 then do;	/* likelihood for survival */
	...
	do k=1 to 4;
		...
		sum2=sum2 + baseh{k} * dur{k} * exp(eta * yall{k});
	end;
	...
	if event=2 then do;
		...
		mu4= alpha1 * z2  + gamma * a + eta * y_current ;
...
ods output ParameterEstimates=est4_all FitStatistics=fit4_all CorrMatParmEst=corr1 CovMatParmEst=cov1; 
run;
...
model fup ~ general(loglik);
random a ~ normal(0, vara) subject=id;
ods output ParameterEstimates=est4 FitStatistics=fit4 CorrMatParmEst=corr&ii CovMatParmEst=cov&ii; 
run;
...
data _null_;
set result;
file "y:\liulei\collaboration\zheng cheng\paper2\data4\AGQresult4.txt";
put alpha1 beta0 beta1 beta2 gamma eta vara  vare h01 h02 h03 h04 
se_alpha1 se_beta0 se_beta1 se_beta2  se_gamma se_eta se_vara  se_vare se_h01 se_h02 se_h03 se_h04  ; 
run;
...
%do ii=1 %to 200;
	data _null_;
	set cov&ii;
	file "y:\liulei\collaboration\zheng cheng\paper2\data4\cov&ii..txt";
	put parameter h01 h02 h03 h04  alpha1 beta0 beta1 beta2 gamma eta vara  vare ; 
	run;
%end;
%mend;
